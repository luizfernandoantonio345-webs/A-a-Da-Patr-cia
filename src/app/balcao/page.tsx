  ...
  return (
    <div style={{ minHeight: "100vh", background: "#160C20" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 20px", background: "#100817" }}>
        <div style={{ display: "grid", placeItems: "center", borderRadius: 12, flex: "none", width: 34, height: 34, background: C.lime }}>
          <ScanLine size={17} color={C.acaiDeep} />
        </div>
        <div style={{ flex: 1 }}>
          <div className="disp" style={{ color: "#fff", fontWeight: 800, fontSize: 16 }}>Balcão · Açaí da Patrícia</div>
          <div style={{ fontSize: 11, color: "#A98FBB" }}>Pedidos em tempo real</div>
        </div>
        {/* ticker da fila */}
        <div style={{ display: "flex", gap: 6 }}>
          {novos > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, fontWeight: 700,
              padding: "5px 10px", borderRadius: 8, background: "rgba(47,125,58,.22)", color: C.lime }}>
              <span className="pulseDot" style={{ width: 7, height: 7, borderRadius: "50%", background: C.lime, display: "inline-block" }} />
              {novos} novo{novos > 1 ? "s" : ""}
            </div>
          )}
          {preparando > 0 && (
            <div style={{ fontSize: 11, fontWeight: 700, padding: "5px 10px", borderRadius: 8,
              background: "rgba(192,138,24,.2)", color: "#E8B130" }}>
              {preparando} prep.
            </div>
          )}
          {prontos > 0 && (
            <div style={{ fontSize: 11, fontWeight: 700, padding: "5px 10px", borderRadius: 8,
              background: "rgba(91,42,136,.25)", color: "#C09FD8" }}>
              {prontos} pronto{prontos > 1 ? "s" : ""}
            </div>
          )}
          {orders.length === 0 && (
            <div style={{ fontSize: 11, fontWeight: 700, padding: "5px 10px", borderRadius: 8,
              background: "rgba(255,255,255,.06)", color: "#A98FBB" }}>
              fila vazia
            </div>
          )}
        </div>
      ...
      <div style={{ display: "flex", gap: 8, padding: "12px 20px" }}>
        {(["fila", "comandas"] as const).map((t) => { const on = tab === t; return (
          <button key={t} onClick={() => setTab(t)} className="press"
            style={{ fontSize: 12, fontWeight: 700, padding: "8px 14px", borderRadius: 8, border: "none",
              background: on ? C.acai : "#241531", color: on ? "#fff" : "#A98FBB" }}>
      ...
      <div style={{ padding: "0 20px 24px" }}>
        {tab === "fila" ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 12 }}>
            {orders.map((o) => (
              <div key={o.id} className="fu" style={{ borderRadius: 16, overflow: "hidden", background: "#211330",
                border: `1px solid ${o.status === "novo" ? C.leaf : "#332145"}` }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
                  padding: "10px 12px", borderBottom: "1px solid #332145" }}>
                  ...
                  <span style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 10, fontWeight: 700,
                    padding: "2px 8px", borderRadius: 4, background: stC[o.status] || "#444", color: "#fff" }}>
                    {o.status === "novo" && <span className="pulseDot" style={{ width: 6, height: 6, borderRadius: "50%", background: "#fff", display: "inline-block" }} />}
                    {stL[o.status] || o.status}
                  </span>
                </div>
                <div style={{ padding: "10px 12px" }}>
                  <div style={{ fontSize: 10, marginBottom: 8, color: "#A98FBB" }}>
                  ...
                  {o.order_items.map((l) => (
                    <div key={l.id} style={{ fontSize: 11.5, marginBottom: 6, color: "#E6D7F0" }}>
                      ...
                      {l.order_item_options.length > 0 && (
                        <em style={{ fontStyle: "normal", display: "block", paddingLeft: 12, fontSize: 10, color: "#C3A4D6" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {comandas.map((cm) => (
              <div key={cm.id} className="fu" style={{ borderRadius: 16, padding: 16, display: "flex", alignItems: "center",
                gap: 16, background: "#211330", border: "1px solid #332145" }}>
                <div style={{ flex: 1 }}>
                  <div className="disp" style={{ color: "#fff", fontWeight: 800, fontSize: 16 }}>Comanda {cm.number}</div>
                  <div style={{ fontSize: 11, marginTop: 2, color: "#A98FBB" }}>{cm.n} itens · {cm.ord} pedido(s)</div>
                </div>
                ...
                <button className="press" onClick={() => fechar(cm.id)}
                  style={{ padding: "10px 16px", borderRadius: 12, fontSize: 12, fontWeight: 700,
                    color: "#fff", border: "none", background: C.leaf }}>
                  Fechar (pago)
                </button>
              </div>
            ))}
...
function BtnFull({ color, onClick, children }: { color: string; onClick: () => void; children: React.ReactNode }) {
  return (
    <button className="press" onClick={onClick}
      style={{ width: "100%", padding: 8, borderRadius: 8, fontSize: 11, fontWeight: 700, color: "#fff", border: "none", background: color }}>
      {children}
    </button>
  );
}
